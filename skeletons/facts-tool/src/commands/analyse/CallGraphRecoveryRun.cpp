...
std::expected<int, std::string> runSelectedCallGraph(
  ...
  auto recovered = recoverCallGraph(options, graph,
      coverage ? std::optional{*coverage} : std::nullopt, rootIds);
  if (!recovered)
    return std::unexpected(recovered.error());
  auto rendered = runCallGraphQuery(options, request, recovered->graph,
      recovered->coverage ? &*recovered->coverage : nullptr, &recovered->report);
  if (!rendered)
    return rendered;

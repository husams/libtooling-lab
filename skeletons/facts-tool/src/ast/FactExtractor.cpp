...
public:
  FactConsumer(FileManager &files, FactStore &store)
      : files_(files), store_(store) {}

  void HandleTranslationUnit(clang::ASTContext &context) override {
    traverse(context, files_, store_);
  }
...
public:
  FactAction(FileManager &files, FactStore &store)
      : files_(files), store_(store) {}

  std::unique_ptr<clang::ASTConsumer>
  CreateASTConsumer(clang::CompilerInstance &, llvm::StringRef) override {
    return std::make_unique<FactConsumer>(files_, store_);
  }
...
public:
  ...
  std::unique_ptr<clang::FrontendAction> create() override {
    return std::make_unique<FactAction>(files_, store_);
  }
...
createFactExtractorFactory(FileManager &files, FactStore &store) {
  return std::make_unique<FactActionFactory>(files, store);
}

...
Result<void> validate(const Query &query) {
  if ((query.qualifiedName.empty() && !query.usr && !query.symbolId) ||
      query.qualifiedName.size() > 4096 ||
  ...
  for (const auto *value : {&query.kind, &query.usr, &query.repository, &query.component, &query.symbolId})

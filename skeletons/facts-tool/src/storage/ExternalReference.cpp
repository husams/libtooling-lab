...
std::expected<bool, std::error_code> Storage::isExternal(SymbolId symbol) {
  ...
  return storage::detail::collectOne(std::move(values))
      .and_then([&](bool value) {
        return commit(*transaction).transform([&] { return value; });
      });
}
...
std::expected<void, std::error_code> Storage::addCallGraphExternalReferences(
  ...
  return database_
      .executeBulk(
          "INSERT INTO callgraph_external_reference(source_id,destination_id,"
          "kind,position,file_id,offset,external_symbol_id) "
          "VALUES(?1,?2,?3,?4,?5,?6,?7)"
          " ON CONFLICT(source_id,destination_id,kind,position,file_id,offset) "
          "DO NOTHING",
          references,
          storage::detail::typedBinder([](auto bind, const auto &value) {
            return bind(value.source, value.destination, value.kind,
                        value.position, value.file, value.offset,
...
Storage::addCallGraphFacts(std::span<const Relation> relations,
  ...
  const auto callers =
      entries |
      std::views::transform([](const auto &entry) { return entry.symbolId; }) |

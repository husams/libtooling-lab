...
Storage::addUnresolvedCallSites(std::span<const UnresolvedCallSite> sites) {
  ...
  return database_
      .executeBulk("INSERT INTO callgraph_unresolved_site(source_id,file_id,"
...
Storage::clearUnresolvedCallSites(std::span<const SymbolId> callers) {
  ...
  return database_
      .executeBulk("DELETE FROM callgraph_unresolved_site WHERE source_id=?1",
                   callers, storage::detail::typedBinder([](auto bind,
                                                             const auto &id) {
                     return bind(id);
                   }))
      .and_then([&](const storage::BulkResult &) { return commit(*transaction); });
...
Storage::clearCallGraphFacts(std::span<const SymbolId> callers) {
  ...
  if (!relations) return std::unexpected(relations.error());
  auto unresolved = clearUnresolvedCallSites(callers);
  if (!unresolved) return std::unexpected(unresolved.error());
  return invalidateCallGraphEntries(callers)

...
Storage::addCallGraphEntries(std::span<const CallGraphEntry> entries) {
  ...
  return database_
      .executeBulk(
          "INSERT INTO callgraph_entry(symbol_id,graph_node_ref) "
          "VALUES(?1,?2) ON CONFLICT(symbol_id) DO UPDATE SET "
          "graph_node_ref=excluded.graph_node_ref",
          entries,
          storage::detail::typedBinder([](auto bind, const auto &entry) {
...
Storage::findCallGraphEntry(SymbolId symbol) {
  ...
  auto values = storage::detail::toItlibGenerator(database_.query(
      ...
      symbol));
  return storage::detail::collectOptional(std::move(values))
      .and_then([&](auto entry) {
        return commit(*transaction).transform([&] { return entry; });
      });
}

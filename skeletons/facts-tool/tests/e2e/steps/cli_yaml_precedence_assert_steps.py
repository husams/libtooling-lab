...
def assert_precedence(defaults, expected, family):
    assert all(
        ...
        for path, before in defaults.precedence_yaml.items()
    )
    ...
    if family == "import":
        ...
        assert bool(options) and ("-DSELECTED=2" in options[0]) == (expected == "cli")
        assert "-DRETAINED=1" not in options[0]
    ...
    if family == "extract":
        with sqlite3.connect(root / "out/extract.db") as connection:
            names = {
                row[0]
                for row in connection.execute("SELECT qualified_name FROM symbol")
            }
    else:
        with (
            sqlite3.connect(root / "out/dependency.db") as output,
            sqlite3.connect(root / "dependency.conf.db") as project,
        ):
    ...
    if family == "extract":
        assert "RetainedYamlDefault" in names
    expected_name = (
        ("YamlHeader" if expected == "yaml" else "CliHeader")
        if family == "extract"
        else ("yaml.hpp" if expected == "yaml" else "cli.hpp")
    )
    omitted = (
        ("CliHeader" if expected == "yaml" else "YamlHeader")
        if family == "extract"
        else ("cli.hpp" if expected == "yaml" else "yaml.hpp")
    )
    if expected == "none":

...
def main() -> None:
    ...
    owned = [
        *sorted((root / "src/analysis/callgraph").glob("*.[ch]pp")),
        *sorted((root / "src/analysis/callgraph").glob("*.h")),
    ...
    require("clang::CallGraph graph" in visitor_text,
    ...
    require("TraverseStmt" not in visitor_text,
            "CallGraphVisitor performs a second body traversal")
    owners = []
    for path in (root / "src").rglob("*.[ch]pp"):

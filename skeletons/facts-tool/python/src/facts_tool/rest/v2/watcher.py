...
class Watcher:
    ...
    def status(self) -> WatcherStatus:
        return decode(WatcherStatus, call(self._http, "GET", path("watcher")))

    def settings(self) -> WatcherSettings:
        return decode(
            WatcherSettings, call(self._http, "GET", path("watcher/settings"))
        )

    def replace_settings(self, settings: WatcherSettings) -> WatcherSettings:
        return decode(
            WatcherSettings, call(self._http, "PUT", path("watcher/settings"), settings)
        )
    ...
    def update_settings(
        ...
        *,
        ...
        return decode(
            WatcherSettings, call(self._http, "PATCH", path("watcher/settings"), body)
        )

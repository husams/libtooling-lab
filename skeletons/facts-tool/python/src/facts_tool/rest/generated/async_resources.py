...
class AsyncResourceOperations:
    ...
    async def find_symbols(
        ...
        return symbol_page(await async_request(self._http, *route))

    async def index_status(self) -> IndexStatus:
        return index_status(await async_request(self._http, *endpoint("indexStatus")))

    async def extract(self, file: FileSelector, *, force: bool = False) -> DomainJob:
        body = extraction_request(file, force)
        route = endpoint("extract")
        return domain_job(await async_request(self._http, *route, body=body))
    ...
    async def dependencies(self, file: FileSelector) -> DomainJob:

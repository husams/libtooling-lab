...
class AsyncClient(AsyncResourceOperations):
    ...
    async def list_jobs(self) -> list[Job | DomainJob]:
    ...
    async def get_job(self, job_id: str) -> Job | DomainJob:
        route = endpoint("getJob", id=identifier(job_id))
        return snapshot(await async_request(self._http, *route))

    async def cancel_job(self, job_id: str) -> Job | DomainJob:

...
def _capture_provenance(
    db: sqlite3.Connection, file_id: int
) -> tuple[str | None, bool]:
    row = db.execute(
        "SELECT path FROM facts_project_provenance WHERE file_id=?", (file_id,)
    ).fetchone()
    return (None, True) if row is None else (str(row[0]), False)
...
def load_expression_occurrences(
    ...
    for source in db.execute(sql):
        ...
        capture_path, provenance_missing = _capture_provenance(db, int(row["file_id"]))
        row.update(
            {
                "id": occurrence_id,
                ...
                "_capture_provenance_missing": provenance_missing,
